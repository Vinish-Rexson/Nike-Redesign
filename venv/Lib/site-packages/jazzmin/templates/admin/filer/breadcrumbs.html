...
<ol class="breadcrumb">
    ...
    <li class="breadcrumb-item">
        <a href="{% url 'admin:filer-directory_listing-root' %}" title="{% trans 'Go back to root folder' %}">{% trans
            "root" %}</a>
    </li>
    {% endif %}
    ...
    <li class="breadcrumb-item">{% if instance.label or instance.name %}
        <a href="{{ instance.get_admin_directory_listing_url_path }}"
           title="{% blocktrans with instance.name as folder_name %}Go back to '{{ folder_name }}' folder{% endblocktrans %}">{%
            if instance.label %}{{ instance.label }}{% else %}{{ instance.name }}{% endif %}</a></li>
    {% endif %}
    <li class="breadcrumb-item active">{{ breadcrumbs_action }}</li>
    {% else %}
    <li class="breadcrumb-item active">{% if instance.label %}{{ instance.label }}{% else %}{{ instance.name }}{% endif
        %}
    </li>
    {% endif %}
</ol>
